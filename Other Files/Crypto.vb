...
#Region "Utility Functions"
    ...
    '********************************************************
    Public Shared Function GetTextFromFile(ByVal fileName As String) As String
...
#Region " FILE Functions "
    Public Sub FileEncript(ByVal Text As String, ByVal OutputFile As String)

        Dim alg As Crypto.Algorithm = 3
        Dim key As String = "theNext"

        Crypto.EncryptionAlgorithm = alg
        Crypto.Key = key
        Crypto.Encoding = Crypto.EncodingType.HEX

        If Crypto.EncryptString(Text) Then
            SaveTextToFile(_content, OutputFile)
        Else
            MsgBox(Crypto.CryptoException.Message)
        End If
        Crypto.Clear()
    End Sub
    Public Function FileDycript(ByVal OutputFile As String) As String
        Dim Result As String = Nothing
        Dim TempFile As String = OutputFile.Substring(0, OutputFile.Length - 3) & "TXT"
        ...
        Crypto.Encoding = Crypto.EncodingType.HEX
        _content = GetFileContentsText(OutputFile)
        If Crypto.DecryptString() Then
            SaveTextToFile(_content, TempFile)
            Result = GetTextFromFile(TempFile)
        Else
            MsgBox(Crypto.CryptoException.Message)
        End If
        Crypto.Clear()
    ...
    Public Function SaveTextToFile(ByVal strData As String, ByVal FullPath As String, Optional ByVal ErrInfo As String = "") As Boolean
        ...
        Return bAns
    End Function
    Public Function GetFileContentsText(ByVal FullPath As String, Optional ByRef ErrInfo As String = "") As String
